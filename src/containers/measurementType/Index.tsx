...
const MeasurementTypeIndex = () => {
  ...
  const loadData = useCallback(async () => {
    let result = await BaseService.getAll<IMeasurementType>(
      "/MeasurementTypes",
      appState.token!
    );
  ...
  const onSubmit = async (data: IMeasurementType) => {
    if (dialogType === EDialogType.Delete) {
      let result = await BaseService.delete<IMeasurementType>(
        "/MeasurementTypes/" + data.id,
        appState.token!
      );
      if (result.ok) {
        setModalState(!modalState);
    ...
    const measurementTypeList = measurementType.filter(
        ...
        {alertMessage ? (
          ...
          <Grid className={"formGrid"}>
            {dialogType === EDialogType.Create ? (
              ...
              <AlertComponent
                message={
                  "Kas olete kindel, et soovite kustutada antud nimetust: "
                }
