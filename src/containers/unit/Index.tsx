...
const UnitIndex = () => {
    ...
    const { handleSubmit, control, setError, setValue, getValues, clearErrors, reset } = useForm<IUnit>({
        defaultValues: {
            name: "",
        },
    });
    ...
    const onSubmit = async (data: IUnit) => {
        if (dialogType === EDialogType.Delete) {
            let result = await BaseService.delete<IUnit>("/Units/" + data.id, appState.token!);
            if (result.ok) {
                setModalState(!modalState);
                await loadData();
            }
                        ...
                        {dialogType === EDialogType.Delete ? null : (
                            <>
                                ...
                                <Controller
                                    ...
                                    rules={{
                                        required: "Lühendi sisestamine on kohustuslik.",
                                    }}
                                />
                            </>
                        )}
