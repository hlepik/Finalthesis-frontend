...
const JoinNewsInputBtn = styled("div")(({ theme }) => ({
  display: "flex",
  flexDirection: "row",
  position: "relative",
...
const PatternIndex = () => {
  ...
  const [searchResult, setSearchResult] = useState<IInstruction[]>(
    [] as IInstruction[]
  );
  ...
  const loadData = useCallback(async () => {
    let response = await BaseService.getAll<ICategory>(
      "/Categories",
      appState.token!
    );
  ...
  const handleCategoryChange = async (category: ICategory) => {
    ...
    let response = await BaseService.getAll<IInstruction>(
      "/Instructions/category/" + category.id,
      appState.token!
    );
        ...
        <MainContainer>
          ...
          {searchResult.map((result: IInstruction) => (
            <InstructionGrid key={result.id}>
              <StyledImage>
                <img
                  src={`${PicturePath}${result.mainPictureName}`}
                  className="Image"
                />
              </StyledImage>
              <TextGrid>
