...
type IconType = "close" | "next" | "previous" | "tick" | "send" | "add" | "delete" | "remove";

type BtnType = "yellow" | "black" | "transparent" | "gray" | "borderGray" | "borderRed" | "red";
...
const getBtnSvgIconByType = (iconType: IconType, color: string, icon: IIcon) => {
    const { height, width, viewBox } = icon;
    const paths = {
        close: (
            <>
                <path d="M16.5 1.5L1.5 16.5" stroke={color} strokeLinejoin="round" strokeWidth="2" />
                <path d="M16.5 16.5L1.5 1.5" stroke={color} strokeLinejoin="round" strokeWidth="2" />
            </>
        ),
        ...
        add: (
            <>
                <path d="M12 3V21" stroke={color} strokeLinejoin="round" strokeWidth="2" />
                <path d="M3 12H21" stroke={color} strokeLinejoin="round" strokeWidth="2" />
            </>
        ),
        remove: (
            <>
                <path d="M0 1H18" stroke={color} strokeLinejoin="round" strokeWidth="2" />
            </>
        ),
        delete: (
            <>
    ...
    const path = !!iconType && !_.isEmpty(iconType) && _.has(paths, iconType) ? _.get(paths, iconType) : null;
...
const StyledBtn = styled(Button, {
    shouldForwardProp: (prop) =>
        prop !== "noLabel" && prop !== "btnType" && prop !== "rotate" && prop !== "paddingSide",
})<{
    noLabel?: boolean;
    btnType: IBtnType;
    rotate?: boolean;
    ...
    textAlign: "center",
    borderRadius: "40px",
    padding: noLabel ? 0 : `16px ${_.isNumber(paddingSide) ? `${paddingSide}px` : "32px"}`,
    height: "3rem",
    width: "auto",
    "&:hover": {
        backgroundColor: btnType.backgroundColor,
...
const btnTypes: {
    ...
    red: {
        ...
        color: "white",
        border: "4px solid rgb(239, 83, 80, 0.3)",
    },
    borderRed: {
        backgroundColor: "transparent",
        color: "#ef5350",
...
const BasicButton: FC<IOutlinedBtn> = ({ label = "", btnType, iconType, noLabel, paddingSide, ...rest }) => {
    const getCorrectBtnType = (btnTypeName: BtnType) => {
        if (!!btnTypeName && !_.isEmpty(btnTypeName) && _.has(btnTypes, btnTypeName)) {
    ...
    const getContent = (btnType: BtnType, label: string, iconType?: IconType, noLabel?: boolean) => {
        ...
        if (!!iconType && !_.isEmpty(iconType) && _.has(iconTypes, iconType)) {
            ...
            const icon = getBtnSvgIconByType(iconType, correctBtnType.color, iconParams);
